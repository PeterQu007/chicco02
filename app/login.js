...
focus = true; setFocusMsg = '<p class="green">setFocusMsg: On</p>';
}
...
let disablePageMsg = '<p class="blue">disablePage: Off by data-disable-page</p>';
let disablePageDiv = '<div id="disable-page-overlay"><div id="page-overlay"><img src="'+cdnUrl+'/images/ajax-e-loading.gif" /><p>'+pleasewait+'..</p></div></div>';
...
let backSpaceClearMsg ='<p class="blue">backSpaceClear: Off by data-backspace-clear</p>';
let backspace = false;
...
let fakeSafariPwdMsg = '<p class="blue">fakeSafariPwd: Off with data-fake-safari-pwd</p>';
let fakepwdadded = false;
let fakepwd = false;
	if (typeof fakeSafariPwdData === 'undefined' ) {
fakepwd = true; fakeSafariPwdMsg = '<p class="green">fakeSafariPwd: On</p>';
}
...
fontawesome = false; fontIconsMsg = '<p class="green">fontIcons: Off or not needed</p>';
}
...
ga('send', 'event', 'Session Dialog', 'Triggered', 'Idp timed out');
		...
		if (useboostrap === true) {
			...
			$('#session-dialog').dialog({title: 'Login Request Expiration Warning', width: 400, modal: true, draggable: false, resizable: false, buttons: {Continue: function() {
$(this).dialog('close'); window.location = redirectUrl;
}}});
		}
	}

let assocDropdownId = $('#assoc');/* loginXkdId.attr('data-assoc-dropdown');  default false */ 
let assocDropdownRememberJsId = $('#rememberjs');
let assocDropdownRememberJsUrl = $('#rememberjs').attr('src');
let assocDropdownRememberJsMsg = '<p class="yellow">assocDropdownRemember: Off remember.min.js Not Found </p>';
let assocDropdownMsg = '<p class="yellow">assocDropdown: Off $("#assoc") not on page</p>';
let assocDropdownCookieMsg = '<p class="yellow">assocDropdownCookie: null no cookie found</p>';
let assocDrop = false;
...
ga('send', 'event', 'KD Script', 'Failed');
		...
		if (window.ga) {
ga('send', 'event', 'KD Script', 'Failed', 'maxKD'+madKD_getVersion()+'.min.js');
}
	} else { // all good
		ssoTypeMsg = '<p class="green">ssoType: KD Version = <strong>maxKD'+madKD_getVersion()+'.min.js</strong></p>';
		didkdload = true;
		if (window.ga) {
ga('send', 'event', 'KD Script', 'Loaded', 'maxKD'+madKD_getVersion()+'.min.js');
}
	}
...
function qaDialog() { // this run on click of the check list image or if a function find an error
	if (useboostrap === true) { // check for bootstrap
		...
		$('#qa-dialog').dialog({title: 'QA Checker Dialog', width: 480, modal: true, draggable: true, resizable: true, buttons: {Close: function() {
$(this).dialog('close');
}}});
	}
}

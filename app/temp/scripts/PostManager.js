...
/***/ (function(module, exports, __webpack_require__) {
	...
	// 		"js": ["jquery-3.2.1.js","postManager.js"]
	// }
	...
	_$$$("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<new postManager.js");
	var Debug = { dbgLocal: 1, dbgBackground: 2, dbgBG: 2, dbgDisable: 3 }; //BG === Background
	chrome.storage.sync.set({ 'postMenuStatus': '' });

	//build two listing objects
	_$$$("postManager.Start.0.0a.build new object: ", backupListing, listingInfo);
	backupListing = new _Listing2.default();
	listingInfo = new _Listing2.default();
	pn = new _Paginator2.default('#paginator', 1);
	_$$$("postManager.0.1.new paginator", pn);
			...
			if (pn.pageChanged !== null) {
					var checkCurrentListing = function checkCurrentListing() {
							...
							if (listingInfo && pn.pageChanged !== null) {
									...
									_$$$('postManager.0.3...create post menu: ');
									cgPostMenu = new _PostMenu2.default(tableSelector, 0);
									cgRepost = new _Repost2.default(pn);

									_$$$("postManager.0.4.currentListing established inside cgRepost is ======> ", listingInfo);
									pn.backupCurrentPage();
									chrome.storage.sync.set({ 'editPageStatus': 'null' });
									cgDelete = new _Delete2.default(pn, Debug.dbgLocal);
					...
					listingInfo = pn.cgTable.currentListing;
					_$$$("postManager.0.2..currentListing inside cgRepost is =========> ", listingInfo);
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var Debug = { enable: 0, disable: 1 // 0: enable, 1: disable
...
/***/ (function(module, exports) {
	...
	var PostMenu = function () {
		function PostMenu(insertAfterSelector, debug) {
			...
			this.delMenuSelector = '';
			this.repMenuSelector = '';
			this.editMenuContent = '';

			this.addPostMenu(this.insertAfterSelector);

			this.addDelMenu(this.delMenuSelector);
			this.addRepMenu(this.repMenuSelector);
		...
		_createClass(PostMenu, [{
			...
			value: function addPostMenu(insertAfterSelector) {
				...
				this.delMenuSelector = '#delmenucontent';
				this.repMenuSelector = '#repmenucontent';
			...
			value: function addEditMenu(menuContent) {
				...
				var $menuOption = $('<input type="checkbox">').attr({ name: "editOption", id: "editSel", value: "editSel" });
				var $lblPostTitle = $('<label>').attr({ id: "editCheckboxLabel" }).text('Edit The Category Posts: ' + 'listingTitle');
				var $lblCurPrice = $('<label>').text('Current Price: ').attr({ id: 'lblCurPrice', name: 'priceLabel' });
				var $txtCurPrice = $('<input type="text">').attr({ id: 'txtCurPrice' });
				var $lblNewPrice = $('<label>').text('New Price: ').attr({ id: 'lblNewPrice', name: 'priceLabel' });
				var $txtNewPrice = $('<input type="text">').attr({ id: 'txtNewPrice' });
				$optContent.append($menuOption).append($lblPostTitle).append($lblCurPrice).append($txtCurPrice).append($lblNewPrice).append($txtNewPrice);

				var $btnContent = $('<div />').attr({ id: "editBtnContent" });
				var $btnOne = $('<input type="submit">').attr({ id: "editCurPostSubmit", value: "edit current post" });
				var $btnCat = $('<input type="submit">').attr({ id: "editSelPostsSubmit", value: "edit selected posts" });
		...
		_createClass(Delete, [{
			...
			value: function bindMenuEvents() {
				...
				(function events(self) {
					...
					self.btnDelOne.click(self.delOnePost.bind(self, Debug.logBG));

					self.btnDelCat.click(self.delCatPosts.bind(self, Debug.logBG));
			...
			value: function delCatPosts() {
				...
				chrome.runtime.sendMessage({ from: 'clsDelete', subject: 'bg.12.fetchBackupPostTitle' }, function (response) {
					...
					delCatPosts = posts.filter(function (post) {

						$$$('clsDelete.deleteCatPosts...post title', post.title);

						return post.title.indexOf(postTitle) != -1;
					});

					$$$('clsDelete.delCatPosts...filter the delete posts: ', delCatPosts);

					self.delCatPosts = delCatPosts;

					$$$('clsDelete.submitDelCatPosts...Posts: ', delCatPosts);

					chrome.runtime.sendMessage({ from: 'clsDelete', subject: 'bg.13.backupCatPosts', catPosts: delCatPosts }, function (response) {

						$$$('clsDelete.submitDelCatPosts...all posts in Cat Deleted');
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var Edit = function () {
		...
		_createClass(Edit, [{
			...
			value: function addEditMenuContent(menuSelector) {

				var self = this;
				var _$$$ = this.log;
				var listingTitle = self.paginator.cgTable.currentListing.title;
				var listingPrice = self.paginator.cgTable.currentListing.price;

				_$$$('classEdit.addEditMenuContent.listingTitle & Price: ', listingTitle, listingPrice);

				self.editCheckboxLabel = (0, _jquery2.default)('#editCheckboxLabel');
				self.editCheckboxLabel.text(listingTitle);
				self.priceText = (0, _jquery2.default)('#priceBox');
				self.priceText.val(listingPrice);
				self.editOneButton = (0, _jquery2.default)('#editCurPostSubmit');
				self.editSelButton = (0, _jquery2.default)('#editSelPostsSubmit');
			}
		}, {
			key: 'checkStatus',
			value: function checkStatus() {
